  ...
  return (
    <div className="w-full">
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        {/* Analytics Card - Full width on mobile/tablet, 2/3 on desktop */}
        <Card className="xl:col-span-2 bg-[#17181c] border-gray-800 flex flex-col h-72">
        ...
        <Card className="bg-[#17181c] border-gray-800 flex flex-col h-72">
          ...
          <CardContent className="flex flex-col justify-between flex-1 overflow-hidden">
            <div className="space-y-3 overflow-y-auto">
              {contacts.map((contact) => (
                ...
                >
                  ...
                  <Button
                    size="sm"
                    className="bg-blue-600 hover:bg-blue-700 text-white text-xs px-3 h-7 flex-shrink-0 ml-2"
                  >
                    Send
                  </Button>
                </div>
            ...
            <Button
              variant="link"
              className="text-blue-400 mt-2 p-0 h-auto self-start flex-shrink-0"
            >
              View All Contacts
            </Button>
          </CardContent>
        ...
        {/* Recent Transactions Card - Full width on mobile/tablet, 2/3 on desktop */}
        <Card className="xl:col-span-2 bg-[#17181c] border-gray-800 flex flex-col h-72">
          <CardHeader className="flex-shrink-0 flex flex-row items-center justify-between">
            <CardTitle className="text-gray-50">Recent Transactions</CardTitle>
            <Button variant="link" className="text-blue-400 flex-shrink-0">
              View All
            </Button>
          </CardHeader>
                ...
                >
                  <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
                    <div
                      className={`p-2 rounded-full flex-shrink-0 ${
                        t.type === "sent" ? "bg-blue-500/20" : "bg-green-500/20"
                      }`}
                    >
                      {t.type === "sent" ? (
                        <ArrowUp className="h-4 w-4 sm:h-5 sm:w-5 text-blue-400" />
                      ) : (
                        <ArrowDown className="h-4 w-4 sm:h-5 sm:w-5 text-green-400" />
                      )}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="font-medium text-sm truncate">
                  ...
                  <p
                    className={`font-semibold text-sm sm:text-base flex-shrink-0 ${
                      t.type === "received" ? "text-green-400" : "text-gray-50"
                    }`}
                  >
        ...
        {/* CTA Card - Full width on mobile/tablet, proper height on all screens */}
        <Card className="bg-[#061622] border-0 p-4 sm:p-6 flex flex-col min-h-[180px] sm:min-h-[200px] xl:min-h-[160px] justify-center items-center">
          <CardTitle className="text-white text-base sm:text-lg text-center">
            Need to pay someone?
          </CardTitle>
          <CardDescription className="text-cyan-100 my-2 sm:my-3 text-xs sm:text-sm text-center">
            Initiate a new transfer or set up a recurring payment.
          </CardDescription>
          <Button
            className="mt-2 w-full sm:w-2/3 xl:w-1/2 bg-gray-50 text-gray-900 font-bold hover:bg-gray-200 h-9 sm:h-10"
